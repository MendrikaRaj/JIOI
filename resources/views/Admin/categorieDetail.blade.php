            ...
            <nav class="navbar navbar-dark align-items-start sidebar sidebar-dark accordion bg-gradient-primary p-0"
                ...
                <div class="container-fluid d-flex flex-column p-0"><a
                    ...
                    <ul class="navbar-nav text-light" id="accordionSidebar" style="color: rgb(0,0,0);">
                        ...
                        <li class="nav-item"><a class="nav-link" href="{{ url('/athlete') }}"
                        ...
                        <li class="nav-item"><a class="nav-link" href="{{ url('/site') }}"
                                style="color: rgba(0,0,0,0.8);"><i class="fas fa-map"
                                    style="color: rgba(0,0,0,0.3);"></i><span>Site</span></a>
                        </li>
                        <li class="nav-item"><a class="nav-link active" href="{{ url('/categorie') }}"
            ...
            <div class="d-flex flex-column" id="content-wrapper">
                <div id="content">
                    ...
                    <section class="contact-clean" style="background: rgba(241,247,252,0);">
                        <form method="post" action="{{ route('enregistrer', ['modelName' => 'CategorieDetail']) }}"
                            ...
                            @endif
                            <h2 class="text-center">Ajouter des recettes/depenses</h2>
                            <div class="mb-3"><label class="form-label">Nom</label><input class="form-control"
                                    type="text" name="nom" placeholder="ex: hebergement">
                                @error('nom')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="mb-3"><label class="form-label">Categorie</label><select
                                    class="form-select" name="categorieid">
                                    <optgroup label="Type ">
                            ...
                            <div class="mb-3"><label class="form-label">Code</label><input class="form-control"
                                    type="text" name="code" placeholder="AAA">
                                @error('code')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                                ...
                                <div class="card-body" style="text-align: left;">
                                    <div class="table-responsive table mt-2" id="dataTable-1" role="grid"
                                        ...
                                        <table class="table my-0" id="dataTable">
                                            ...
                                            <tbody>
                                                @foreach ($categorieDetail as $categorieDetails)
                                                    <tr>
                                                        <td>{{ $categorieDetails->nom }}</td>
                                                        @foreach ($categorie as $categories)
                                                            @if ($categorieDetails->categorieid == $categories->id)
                                                                <td>{{ $categories->type }}</td>
                                                            @endif
                                                        @endforeach
                                                        <td>{{ $categorieDetails->code }}</td>
                                                        <td style="text-align: left;">
                                                            <button class="btn btn-link" type="button"
                                                                style="border: none; color: blue; text-decoration: underline;"
                                                                data-bs-toggle="modal"
                                                                data-bs-target="#modification-modal-{{ $categorieDetails->id }}">
                                                                Modifier
                                                            </button>
                                                        </td>
                                                        <td style="text-align: left;">
                                                            <button class="btn btn-link" type="button"
                                                                ...
                                                                data-bs-toggle="modal"
                                                                data-bs-target="#suppression-modal-{{ $categorieDetails->id }}">
                                                                Supprimer
                                                            </button>
                                                        </td>
                                                    </tr>
                                                    <div class="modal fade" role="dialog" tabindex="-1"
                                                        id="modification-modal-{{ $categorieDetails->id }}">
                                                        <div class="modal-dialog" role="document">
                                                            <div class="modal-content"
                                                                ...
                                                                <div class="modal-body">
                                                                    ...
                                                                    <form
                                                                        action="{{ route('modifier', ['modelName' => 'CategorieDetail', 'id' => $categorieDetails->id]) }}"
                                                                        ...
                                                                        <div class="mb-3"><label
                                                                                class="form-label">Catégorie</label><select
                                                                                class="form-select"
                                                                                name="categorieid">
                                                                                <optgroup label="Type">
                                                                                    @foreach ($categorie as $categories)
                                                                                        <option
                                                                                            value="{{ $categories->id }}"
                                                                                            {{ $categorieDetails->categorieid == $categories->id ? 'selected' : '' }}>
                                                                                            {{ $categories->type }}
                                                                                        </option>
                                                                                    @endforeach
                                                                                </optgroup>
                                                    ...
                                                    <div class="modal fade" role="dialog" tabindex="-1"
                                                        id="suppression-modal-{{ $categorieDetails->id }}">
                                                        <div class="modal-dialog" role="document">
                                                            <div class="modal-content"
                                                                style="background-color: white">
                                                                <div class="modal-header" style="text-align: left;">
                                                                ...
                                                                <div class="modal-body">
                                                                    <form method="POST"
                                                                        action="{{ route('supprimer', ['modelName' => 'CategorieDetail', 'id' => $categorieDetails->id]) }}">
                                                                        @csrf
                                                                        @if (Session::has('success'))
                                                                            <div class="alert alert-success">
                                                                                {{ Session::get('success') }}</div>
