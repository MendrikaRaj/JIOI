            ...
            <nav class="navbar navbar-dark align-items-start sidebar sidebar-dark accordion bg-gradient-primary p-0"
                ...
                <div class="container-fluid d-flex flex-column p-0"><a
                    ...
                    <ul class="navbar-nav text-light" id="accordionSidebar" style="color: rgb(0,0,0);">
                        ...
                        <li class="nav-item"><a class="nav-link active" href="{{ url('/calendrier_User') }}"
                                style="color: rgba(0,0,0,0.8);"><i class="fas fa-calendar-alt"
                                    style="color: rgba(0,0,0,0.3);"></i><span>Calendrier</span></a>
                        </li>
                        <li class="nav-item"><a class="nav-link" href="{{ url('/medaille') }}"
                                ...
                                <section class="newsletter-subscribe">
                                    <div class="container">
                                        <form class="d-flex justify-content-center flex-wrap" method="post"
                                            action="{{ route('filtreCalendrier') }}">
                                            @csrf
                                            <div class="mb-3"><input class="form-control" type="date"
                                                    name="date">
                                            </div>
                                            <div class="mb-3"><select class="form-control" name="disciplineid">
                                                    <optgroup label="Discipline">
                                                        <option value=""></option>
                                                        @foreach ($discipline as $disciplines)
                                                            <option value="{{ $disciplines->id }}">
                                                                {{ $disciplines->nom }}</option>
                                                        @endforeach
                                                    </optgroup>
                                                </select></div>
                                    ...
                                    <table class="table my-0" id="dataTable">
                                        ...
                                        <tbody>
                                            @foreach ($calendrier as $calendriers)
                                                <tr>
                                                    <td>{{ $calendriers->discipline }}</td>
                                                    <td>{{ $calendriers->site }}</td>
                                                    <td>{{ \App\Models\FormatDate::formatFR($calendriers->date) }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                        <tfoot>
